...
在 **AI 模型 → 云端 → Doubao Speech → 识别选项** 中可以调整[豆包官方流式协议](https://docs.volcengine.com/docs/6561/1354869?lang=zh)及[请求参数说明](https://docs.volcengine.com/docs/6561/2628951?lang=zh)支持的参数：二遍识别、文本规范化（ITN）、自动标点、语义顺滑、首字加速及静音判停阈值。首字加速档位范围为 0–20，数值越高首字返回越快，但早期结果准确率可能降低；静音判停阈值范围为 300–5000 ms，数值越低越快确定分句，也越可能提前切句。`show_utterances` 和 `result_type=full` 是 VoiceInk 正确合并流式结果所需的内部参数，因此不提供关闭开关。

这些识别选项不含密钥，会由 VoiceInk 的 iCloud 配置同步传到其他 Mac。API Key 仍然只保存在每台 Mac 的 Keychain 中；在其他设备使用同步过来的豆包模式或识别选项前，仍需先在当地设备配置密钥。

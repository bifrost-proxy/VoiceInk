...
在 **AI 模型 → 云端 → Doubao Speech → 识别选项** 中可以调整[豆包官方流式协议](https://docs.volcengine.com/docs/6561/1354869?lang=zh)及[请求参数说明](https://docs.volcengine.com/docs/6561/2628951?lang=zh)支持的参数：二遍识别、POI 地图领域辅助识别、音乐领域辅助识别、文本规范化（ITN）、自动标点、语义顺滑、首字加速及静音判停阈值。POI 和音乐领域辅助识别仅在二遍识别开启时生效；POI 可选填地级市提示，并与 VoiceInk 词典热词合并到同一份识别上下文。首字加速档位范围为 0–20，数值越高首字返回越快，但早期结果准确率可能降低；静音判停阈值范围为 300–5000 ms，数值越低越快确定分句，也越可能提前切句。`show_utterances` 和 `result_type=full` 是 VoiceInk 正确合并流式结果所需的内部参数，因此不提供关闭开关。

除 POI 城市提示外，这些识别选项不含密钥，会由 VoiceInk 的 iCloud 配置同步传到其他 Mac。POI 城市可能透露位置或出行意图，因此只保存在当前 Mac。API Key 仍然只保存在每台 Mac 的 Keychain 中；在其他设备使用同步过来的豆包模式或识别选项前，仍需先在当地设备配置密钥。

如需在悬浮窗中查看实时文字，请确认 **设置 → 实时文本显示** 已开启。VoiceInk 的“词典”支持普通词汇和专有词两种条目。VoiceInk 会按照火山官方协议，把两类条目的词文本写入 `request.corpus.context` 内的 `hotwords`，随每次豆包连接发送，以改善专有名词识别。

“词替换”只负责在最终转写后执行确定性的“原词 → 目标文本”本地替换，不会作为热词发送。火山接口的 `correct_table_id` / `correct_table_name` 只接受在火山自学习平台创建的替换词表，不能直接接收 VoiceInk 的全局映射；因此 VoiceInk 不会伪造这些字段，也不需要你额外在火山控制台维护同一份词表。

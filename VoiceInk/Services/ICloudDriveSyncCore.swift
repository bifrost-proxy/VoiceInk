...
final class ICloudDriveSyncCore: @unchecked Sendable {
    ...
    init(
        ...
        maximumPayloadBytes: Int = ICloudDriveSyncCore.maximumPayloadBytes
    ) {
    ...
    func append(_ batch: VoiceInkSyncMutationBatch, domain: VoiceInkSyncDomain) throws -> VoiceInkSyncEnvelope {
        ...
        guard payload.count <= maximumPayloadBytes else { throw POSIXError(.EFBIG) }
    ...
    func appendChunked(
        ...
        while start < mutations.endIndex {
            guard try encodedSize(mutations[start...start]) <= maximumPayloadBytes else {
            ...
            while lowerBound <= upperBound {
                ...
                let fits = try encodedSize(mutations[start..<candidateEnd]) <= maximumPayloadBytes
                if fits {
